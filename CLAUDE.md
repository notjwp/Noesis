...
| Definition of Done | **7/9**, audited 2026-09-25 against evidence rather than memory - it read 9/9 while §10 itself had one tick in nine. Unmet: NFR-402 (median 68,750 tokens since 09-20 against 60,000; the all-time median is 41,020, so it is a REGRESSION as the toolset grew) and NFR-302's recovery half ON WINDOWS. Must-have requirements **35/35**; outstanding overall: FR-205 (reverted), FR-503/504 (deliberately unmet), FR-704 `[W]` |
...
- **`os.kill(pid, 0)` does not prove death on Windows.** It raises `OSError WinError 87`
  where Linux raises `ProcessLookupError`, so `_alive()` - which catches OSError and
  returns True to fail safe - reports a dead worker as alive forever, and `recover()`
  never requeues. `_pid_started()` reads /proc, so it returns None here and cannot
  break the tie. Found 2026-09-25 by actually killing a worker; seven passing tests
  and a Linux container both said it worked. Kill the process, do not read the code.
