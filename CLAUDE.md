...
(a kernel-enforced sandbox for scored runs only), CLI and Textual TUI, task queue, cron scheduler, email channel, web search, measurement rig. **1,184 offline tests**, green with no API key, no network, a
...
| Phase R (skill revision) | **DELETED 2026-09-23**, and the defect it existed to fix was in RETRIEVAL. R marked a skill suspect when a run ended badly and let the next `done` run overwrite it: it marked on 7 runs and **6 of them PASSED**, so it was off by default from 2026-09-13 and never ran in real use. Reading the fixture settles it - with R off, extraction already writes `runbook` beside the stale `conventions`, so the library ends CORRECT; the control arm scored 1/3 because `best_match` returned None on a tie and **nothing was injected**. So: no suspect store, no trigger, no overwrite (the `skill_failures` table is dropped by migration v5), and `best_match` breaks a tie by RECENCY - the skill written from the most recently read document wins; equal write times are still no match, which is what stops the rule becoming "always inject something". A net deletion: 1,200 -> 1,184 tests, ~280 lines gone. **UNMEASURED** - pre-registered bar `revision` 3/3 against the 1/3 control, guards `skills` 18/18 and `authoring` 11/11, and verified in the skill FILES (both skills present, `conventions` still `-slate`, `runbook` injected) because a pass with the wrong skill injected is a pass for the wrong reason. Earlier: real 3/3, control 1/3 on 2026-09-11, kept on a pre-registered condition |
...
pytest                            # 1,184 tests, no API key, no network
```
