...
| real repositories | **11/18** at 3 runs per case (2026-09-19), a re-baseline with no change under test, FLAT with 10/18 on 2026-09-03: the +1 is `humanize[1]` landing a good seed (0 in eight passes before, 1/3 now, `budget` verdict with green tests) and nothing shipped since touches these cases' path. `rich` 0/3, losses all `budget`. What moved and is NOT explained: median tokens **402k** against 236-242k, `budget` on 10/18 against 4/18, and 5 of 11 passes not ending by `done` - the growth is TURNS (median 17.5 -> 25.5, tokens per turn flat), mostly `read_file` re-reads of one file before the first edit. Not the gate (0 new escalations), not the schema, not the floor or bound (both in the 09-12 pass at 236k). Seed variance or endpoint drift; n=3 cannot separate them. A cycle only if it repeats on unchanged code; if so the bucket is passes that do not terminate, which is `reflect`. Before: 8/12 at 2 runs, twice (2026-09-12), after the compaction bound (4,000 chars, derived from 87 recorded) and the `read_file` floor (100 lines), both kept on pre-registered conditions; five 2-run passes 7, 7, 8, 8, 8 of 12 |
