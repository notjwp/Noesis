...
(a kernel-enforced sandbox for scored runs only), CLI and Textual TUI, task queue, cron scheduler, email channel, web search, measurement rig. **1,197 offline tests**, green with no API key, no network, a
...
| real repositories | **9/18** at 3 runs per case (2026-09-19, the second pass that day); three 3-run passes on the same loop code read **10, 11, 9** - FLAT, the spread is the seed. `rich` 0/3 four passes running and `humanize` 0/3 (its 1/3 that morning was a seed), every loss `budget`; the four winnable cases 9/12, 7/8, 10/12, 9/12. What DID move, replicated on identical code: median tokens **~390-400k** against ~240k on 09-12, `budget` on 9-10 of 18 against 4 - the winnable four went 155k -> 306k -> 229k with `budget` 1 -> 4 -> 3, by TURNS (tokens per turn flat at ~13k), mostly `read_file` re-reads before the first edit. The morning's reading, "passes that do not terminate", did NOT replicate (1 of 9 against 5 of 11). `git diff e65d94b..f71efe2` over the loop, prompts and fixtures is docstrings and R's default: nothing in the tree moved it, which leaves the endpoint serving something different under the same model id. **Next, before any tuning: the control** - run `real` on `e65d94b` now; ~400k rules the repository out, ~155k means the "cosmetic" diff is not. Before: 8/12 at 2 runs, twice (2026-09-12), after the compaction bound (4,000 chars, derived from 87 recorded) and the `read_file` floor (100 lines), both kept on pre-registered conditions; five 2-run passes 7, 7, 8, 8, 8 of 12 |
...
pytest                            # 1,197 tests, no API key, no network
```
